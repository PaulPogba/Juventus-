
//
//  AJBFindSecretScrollView.m
//  AiJiaBao
//
//  Created by 医麦思 on 2017/2/23.
//  Copyright © 2017年 nuhgroup. All rights reserved.
//

#import "AJBFindSecretScrollView.h"
#import "AJBFindSecretFirstView.h"
#import "AJBFindSecretSecondView.h"
#import "AJBFindSecretThreeView.h"

@interface AJBFindSecretScrollView ()<AJBFindSecretViewDelegate,AJBRegisterSecondViewDelegate>

@property (nonatomic, strong) AJBFindSecretFirstView *firstView;
@property (nonatomic, strong) AJBFindSecretSecondView *secondView;
@property (nonatomic, strong) AJBFindSecretThreeView *threeView;

@end

@implementation AJBFindSecretScrollView

- (instancetype)init
{
    if (self = [super init]) {
        
        __weak typeof (self) __weakSelf = self;
        
        [self addSubview:self.firstView];
        [self.firstView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(0);
            make.left.mas_equalTo(0);
            make.width.mas_equalTo(SCREEN_WIDTH);
            make.height.mas_equalTo(SCREEN_HEIGHT - NAVITETION_HEIGHT);
        }];
        
        [self addSubview:self.secondView];
        [self.secondView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(0);
            make.left.mas_equalTo(__weakSelf.firstView.mas_right);
            make.width.mas_equalTo(SCREEN_WIDTH);
            make.height.mas_equalTo(SCREEN_HEIGHT - NAVITETION_HEIGHT);
        }];
        
        [self addSubview:self.threeView];
        [self.threeView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(0);
            make.left.mas_equalTo(__weakSelf.secondView.mas_right);
            make.width.mas_equalTo(SCREEN_WIDTH);
            make.height.mas_equalTo(SCREEN_HEIGHT - NAVITETION_HEIGHT);
        }];
    }
    return self;
}

#pragma mark - getters and setters
- (AJBFindSecretFirstView *)firstView
{
    if (!_firstView) {
        self.firstView = [[AJBFindSecretFirstView alloc] init];
        _firstView.delegate = self;
    }
    return _firstView;
}

- (AJBFindSecretSecondView *)secondView
{
    if (!_secondView) {
        self.secondView = [[AJBFindSecretSecondView alloc] init];
        _secondView.delegate = self;
    }
    return _secondView;
}


- (AJBFindSecretThreeView *)threeView
{
    if (!_threeView) {
        self.threeView = [[AJBFindSecretThreeView alloc] init];
    }
    return _threeView;
}

#pragma mark - delegate
- (void)findSecretFirstNextAction:(UIButton *)sender
{
    [_secondView  refreshPhoneLabel:_firstView.phoneString];
    __weak typeof(self) __weakSelf = self;
    [UIView animateWithDuration:0.3 animations:^{
        __weakSelf.contentOffset = CGPointMake(SCREEN_WIDTH, 0);
    }];
}

- (void)secondNextAction:(UIButton *)sender
{
    __weak typeof(self) __weakSelf = self;
    [UIView animateWithDuration:0.3 animations:^{
        __weakSelf.contentOffset = CGPointMake(SCREEN_WIDTH*2, 0);
    }];
}


@end
